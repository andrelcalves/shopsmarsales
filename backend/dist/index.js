...
function resolveTraySubSource(row, orderId) {
    ...
    if (storeVal != null && String(storeVal).trim()) {
        ...
        if (d === TRAY_STORE_ID_ATACADO)
            return TRAY_SOURCE_ATACADO;
        if (d === TRAY_STORE_ID_VAREJO)
            return TRAY_SOURCE_VAREJO;
    }
    ...
    if (first === '2')
        return TRAY_SOURCE_VAREJO;
    if (oid)
        console.warn('[tray] Canal não identificado (nem loja nem prefixo 5/2); gravando como tray legado:', oid);
    return 'tray';
}
...
function feeChannelForTrayOrder(orderSource, orderId) {
    if (orderSource === TRAY_SOURCE_ATACADO || orderSource === TRAY_SOURCE_VAREJO)
        return orderSource;
    if (orderSource === 'tray')
        return resolveTraySubSource(null, orderId);
    return orderSource;
}
function channelLabelForChart(source) {
    switch (source) {
        case 'shopee':
            return 'Shopee';
        case 'tiktok':
...
function parseDateOnlyAsNoonUTC(dateStr) {
...
function standardizeShopeeRow(row) {
    try {
...
function monthStartFromYYYYMM(v) {
    const s = String(v || '').trim();
...
function hashToUniqueId(s) {
    let h = 0;
    for (let i = 0; i < s.length; i++)
        h = ((h << 5) - h + s.charCodeAt(i)) | 0;
    return String(Math.abs(h));
}
async function ensureProduct(prisma, source, productCode, name, opts) {
    const code = `${source}_${String(productCode).trim()}`;
    const extra = {};
    if (opts?.variationName)
        extra.variationName = opts.variationName;
    if (opts?.parentCode)
        extra.parentCode = opts.parentCode;
    if (opts?.sku)
        extra.sku = opts.sku;
...
async function autoGroupVariations(prisma, parentCode, groupName) {
    const products = await prisma.product.findMany({
        where: { parentCode },
        include: { productGroupItem: true },
    });
    const ungrouped = products.filter((p) => !p.productGroupItem);
    ...
    const fullGroupName = variations.length > 0
        ...
        : groupName;
    const alreadyGrouped = products.find((p) => p.productGroupItem);
    let groupId;
    if (alreadyGrouped) {
...
async function main() {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            try {
                ...
                if (source === 'shopee') {
                    ...
                    const parentNames = new Map(); // parentCode → baseName
                    for (const r of rows) {
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            try {
                ...
                for (const it of items) {
                    ...
                    if (!prev)
                        byOrder.set(k, { qty: it.quantity, firstName: it.name });
                    else
                        byOrder.set(k, { qty: prev.qty + it.quantity, firstName: prev.firstName });
                }
                const productIds = new Map();
                for (const it of items) {
                ...
                const orderIdToSources = new Map();
                for (const it of items) {
                    if (!orderIdToSources.has(it.orderId))
                        orderIdToSources.set(it.orderId, new Set());
                ...
                for (const [orderId, agg] of byOrder.entries()) {
                    ...
                    for (const src of list) {
                        ops.push(prisma.order.updateMany({
                ...
                const results = await prisma.$transaction(ops);
                const updatedOrders = results.filter((r) => r && typeof r.count === 'number').reduce((a, r) => a + r.count, 0);
                return res.status(200).json({
    ...
    app.post('/api/products', async (req, res) => {
        try {
            ...
            if (!c || !n)
                return res.status(400).json({ message: 'code e name obrigatórios.' });
    ...
    app.put('/api/products/:id', async (req, res) => {
        try {
            ...
            if (costPrice !== undefined)
                data.costPrice = costPrice === null ? null : parseBrNumber(costPrice);
    ...
    app.get('/api/adspend', async (req, res) => {
        try {
            ...
            if (from || to) {
                ...
                if (to)
                    where.month.lt = new Date(to.getFullYear(), to.getMonth() + 1, 1);
            }
            const prismaAny = prisma;
    ...
    app.post('/api/adspend', async (req, res) => {
        try {
            ...
            if (!month)
                return res.status(400).json({ message: 'month inválido. Use YYYY-MM (ex: 2026-01).' });
            if (!channel)
    ...
    app.get('/api/inventory-config', async (_req, res) => {
        try {
            ...
            if (!row)
                return res.status(200).json({ stockStartDate: null });
    ...
    app.post('/api/inventory-config', async (req, res) => {
        try {
            ...
            if (!dateStr)
                return res.status(400).json({ message: 'stockStartDate obrigatório (YYYY-MM-DD).' });
    ...
    app.post('/api/product-groups', async (req, res) => {
        try {
            const prismaAny = prisma;
            if (!prismaAny.productGroup)
                return res.status(500).json({ message: 'Model ProductGroup não disponível.' });
            const { name, productIds } = req.body ?? {};
            const nameStr = typeof name === 'string' ? name.trim() : '';
            const ids = Array.isArray(productIds) ? productIds.map((x) => parseInt(String(x), 10)).filter((n) => Number.isInteger(n) && n > 0) : [];
            if (!nameStr)
                return res.status(400).json({ message: 'Nome do grupo é obrigatório.' });
            if (ids.length < 2)
                return res.status(400).json({ message: 'Selecione pelo menos 2 produtos para formar um grupo.' });
            ...
            if (e?.code === 'P2002')
                return res.status(400).json({ message: 'Um dos produtos já pertence a outro grupo.' });
            console.error(e);
    ...
    app.patch('/api/product-groups/:id', async (req, res) => {
        try {
            ...
            if (Array.isArray(productIds)) {
                const ids = productIds.map((x) => parseInt(String(x), 10)).filter((n) => Number.isInteger(n) && n > 0);
                if (ids.length >= 2) {
            ...
            if (updates.name)
                await prismaAny.productGroup.update({ where: { id: gid }, data: { name: updates.name } });
    ...
    app.put('/api/product-group-stock', async (req, res) => {
        try {
            ...
            if (!Number.isInteger(gid) || gid <= 0)
                return res.status(400).json({ message: 'productGroupId inválido.' });
    ...
    function isOrderValidForStock(o) {
        const st = String(o.status || '').toLowerCase();
        if (st.includes('cancelado'))
            return false;
        if (st.includes('não pago') || st.includes('nao pago'))
            return false;
        return true;
    }
    ...
    const updateBillStatus = async (billId) => {
        ...
        if (!bill)
            return;
        const totalPaid = (bill.payments || []).reduce((s, p) => s + (p.paidAt != null ? (p.amount || 0) : 0), 0);
        let status = 'pending';
    ...
    app.patch('/api/bills/:id', async (req, res) => {
        try {
            ...
            if (isFixedCost !== undefined)
                data.isFixedCost = isFixedCost === true || isFixedCost === 'true';
    ...
    app.post('/api/bills/:id/payments', async (req, res) => {
        try {
            ...
            if (amount <= 0)
                return res.status(400).json({ message: 'Valor da parcela deve ser maior que zero.' });
            const dueDate = parseDateOnlyAsNoonUTC(dueDateStr);
            if (!dueDate)
    ...
    app.get('/api/stock-projection', async (_req, res) => {
        try {
            ...
            const avgPriceByProduct = new Map();
    ...
    app.get('/api/ads-dashboard', async (req, res) => {
        try {
            ...
            if (from || to) {
                ...
                if (from)
                    orderWhere.orderDate.gte = from;
                if (to)
                    orderWhere.orderDate.lt = new Date(to.getFullYear(), to.getMonth() + 1, 1);
            }
            const orders = await prisma.order.findMany({
                where: orderWhere,
                select: { orderDate: true, source: true, totalPrice: true, status: true },
            });
            const revenueByMonthChannel = new Map(); // YYYY-MM|channel
            const revenueByMonthTotal = new Map(); // YYYY-MM
            ...
            const spendByMonthChannel = new Map(); // YYYY-MM|channel
            ...
            const months = new Set([...revenueByMonthTotal.keys(), ...spendByMonthTotal.keys()]);
            const monthsSorted = [...months].sort();
            const channels = new Set();
            for (const k of revenueByMonthChannel.keys())
                channels.add(k.split('|')[1]);
            for (const k of spendByMonthChannel.keys())
                channels.add(k.split('|')[1]);
    ...
    app.get('/api/shopee/auth-url', async (req, res) => {
        try {
            const integration = await prisma.shopeeIntegration.findFirst({ orderBy: { id: 'desc' } });
            if (!integration)
                return res.status(400).json({ message: 'Configure Partner ID e Partner Key primeiro.' });
            const redirectUrl = `${req.protocol}://${req.get('host')}/api/shopee/callback`;
            const url = shopeeApi.buildAuthUrl(Number(integration.partnerId), integration.partnerKey, redirectUrl);
    ...
    app.get('/api/shopee/callback', async (req, res) => {
        try {
            ...
            const tokenRes = await shopeeApi.getAccessToken(Number(integration.partnerId), integration.partnerKey, code, Number(shopId));
            if (tokenRes.error) {
            ...
            let shopName = '';
            try {
                const info = await shopeeApi.getShopInfo(Number(integration.partnerId), integration.partnerKey, tokenRes.access_token, Number(shopId));
    ...
    app.post('/api/shopee/refresh-token', async (_req, res) => {
        try {
            ...
            const tokenRes = await shopeeApi.refreshAccessToken(Number(integration.partnerId), integration.partnerKey, integration.refreshToken, Number(integration.shopId));
            if (tokenRes.error) {
                await prisma.shopeeIntegration.update({
                    where: { id: integration.id },
                    data: { status: 'expired' },
    ...
    app.post('/api/shopee/sync', express.json(), async (req, res) => {
        try {
            ...
            const orderSns = await shopeeApi.fetchAllOrderSns(partnerId, integration.partnerKey, integration.accessToken, shopId, timeFrom, timeTo);
            ...
            const orders = await shopeeApi.fetchOrderDetails(partnerId, integration.partnerKey, integration.accessToken, shopId, orderSns);
    ...
    app.post('/api/returns', express.json(), async (req, res) => {
        try {
            ...
            if (e?.code === 'P2002')
                return res.status(400).json({ message: 'Este pedido já possui uma devolução registrada.' });
            console.error(e);
    ...
    app.post('/api/products/consolidate', express.json(), async (req, res) => {
        try {
            const { productIds, groupName, groupId } = req.body;
            ...
            const skuSource = groupProducts.find((p) => p.sku);
            if (skuSource) {
