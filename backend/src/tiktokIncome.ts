...
/** Soma valores absolutos (alocação por SKU no mesmo pagamento). */
function sumAbs(values: number[]): number {
  return round2(values.reduce((acc, v) => acc + Math.abs(v), 0));
}

/**
 * Várias linhas com o mesmo valor: pode ser total do pedido repetido (deduplicar)
 * ou parcela igual por SKU (somar). Usa contagem de SKUs distintos como critério.
 */
function dedupeOrSumAbs(values: number[], lines: Record<string, unknown>[]): number {
  ...
  // Vários SKUs no mesmo pagamento → valores por linha devem ser somados
...
function aggregatePaymentBucket(lines: Record<string, unknown>[]): {
  ...
  const taxesFromCol = dedupeOrSumAbs(taxesTotal, lines);
  const commissionFee = dedupeOrSumAbs(platformComm, lines);
  let serviceFee = dedupeOrSumAbs(lineServiceFees, lines);
  ...
  const partnerCommission = dedupeOrSumAbs(partnerParts, lines);

  return {
    settlementAmount: dedupeOrSumAbs(settlements, lines),
    commissionFee,
    serviceFee,
    partnerCommission,
